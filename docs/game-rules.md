...
After a successful day, bank the unused-thwack payout and choose one of three free bird offers. The offered birds may be any quality. Then review the complete flock and optionally remove birds for £3 each before the next flock loads. A failed day awards no cash, bird offer, or shop visit and is retried with the same flock and deterministic shuffle sequence.
...
- The player must choose exactly one offered bird before starting the next day. Choosing it adds one bird and therefore one egg to each subsequent daily pool without spending cash.
- The flock overview shows every owned bird individually. Selecting a bird there removes that exact flock entry for £3. Removal is unavailable when cash is insufficient or only one bird remains.
- The day reward and flock overview show the next day's target before selection.
...
- **Bird offer:** A mandatory choice of one free Chicken, Cuckoo, Sparrow, Plover, or Spoonbill after a successful day. The three candidates may have different quality tiers. Choosing one adds exactly one bird and one egg to each subsequent daily pool.
...
- **Shop:** The post-success flock overview where the only current paid action is removing a selected bird for £3. Leaving preserves unspent cash.
