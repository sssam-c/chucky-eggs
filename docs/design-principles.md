...
On the hairpin, use one upright linked carriage per screen column. One rigid brass spine carries two modest versions of the familiar spoon bowl, one beside each row; the entire carriage throws once so both bowls visibly contact their eggs together. Keep stored bowls in the gaps between columns so eggs remain readable. Do not put eggs in buckets. The upper and lower conveyor casings touch so the return reads as one compact machine rather than two remote rows.

Present input as physical levers rather than labelled button faces. Days 1–2 use the three established circuit levers. The hairpin places five levers directly beneath their towers and gives each a separate coloured conduit and symbol. Green and Purple currently distinguish column controls rather than promising additional damage rules; Pink's spark remains the only circuit-specific weakness. Spatial alignment, synchronized throw, and latch sound should make the operated machinery feel causally connected.
