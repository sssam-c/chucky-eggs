...
On the hairpin, use one familiar wall-hinged utensil per screen column. Fix two modest spoon bowls at different distances along one continuous handle. At rest both bowls face outward above the upper egg; on a thwack the entire utensil tips straight toward the player without a lateral sweep. Present that turn as a short sequence of authored whole-utensil poses rather than independently interpolating its bowls and shaft. Every falling pose keeps the shaft visibly overlapped with both bowls; only the final contact pose uses egg occlusion to tuck the shaft behind the eggs. At the brief edge-on overlap, use perspective scale, draw order, and a brass collar on the original far bowl to preserve its identity as it grows into the lower/front impact bowl. Preserve the upright bowl proportions through most of that turn, then flatten them for the clean final contact. A slight farther-bowl-first clink may sell depth, but it is feedback only: both direct hits remain one simultaneous resolver-authored batch. Do not put eggs in buckets. The upper and lower conveyor casings touch so the return reads as one compact machine rather than two remote rows.

Present input as physical levers rather than labelled button faces. Days 1–2 use the three established circuit levers. The hairpin places five levers directly beneath their spoons and gives each a separate coloured conduit and symbol. Green and Purple currently distinguish column controls rather than promising additional damage rules; Pink's spark remains the only circuit-specific weakness. Spatial alignment, synchronized throw, and the two close clinks should make the operated machinery feel causally connected.
