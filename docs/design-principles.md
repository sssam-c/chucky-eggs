...
Present input as physical levers rather than labelled button faces. Lever mappings expose which two column spoons and which target length a circuit selects; the same physical spoon may therefore serve one upper and one lower circuit. Pink's spark identifies both the bend control and the only circuit-specific weakness. The shared upright resting language and Pink's complete two-stage motion should make the operated machinery feel causally connected.
