...
Give the abstract daily target a visible recipient. Present awarded egg points as proportional progress toward Grandma's appetite, while retaining the exact numeric value for overflow and non-colour readability. Group Grandma, appetite progress, one prominent Belt Condition bar, and settings in one right-hand machine rail so persistent character and operational information occupy a single place without taking a band from the playfield. The rail owns only presentation: the session remains the canonical owner of score, target, Belt Condition, and day-end resolution. Reserve her portrait area for replaceable animation, status, and dialogue without letting those timings decide gameplay.
