...
## D075 — Adopt five-tap Hunger rounds as the authoritative game

**Status:** accepted

**Decision:** Replace the conveyor game as current authority with one five-cup tabletop round. Give every cup one directly clickable spoon with fixed colours Red, Blue, Pink, Red, and Blue. A selected occupied spoon directly damages only its egg; adjacent Cuckoos copy that damage, Pink deals 2 direct damage to Spoonbill, and a surviving directly tapped Plover swaps one cup left. Resolve the complete damage and hatch cascade before visibly refilling opened cups from the ordered hopper into those exact vacancies. Grandma begins with 10 Hunger; Yolk reduces it toward zero. The player receives five paid taps per phase. If the fifth complete tap has not won or exhausted the eggs, Grandma adds her announced Hunger increase, that future increase grows by 1, and all five taps refresh. Present Taps as the player's opportunity and keep Grandma, her bowl, current Hunger, next increase, and response together in a persistent right sidebar. Make this scene the project default and treat it as the canonical rule baseline. Keep the current authored twelve-egg order and exact visible toughness and Yolk while the first strategic playtest question is evaluated. Defer conveyor movement, the bin, Belt Condition, Patience, paired circuits, flocks, shops, economy, multiple days, quality, and permanent progression.

**Reason:** Iterating from conveyor movement and global action limits toward direct individual spoons, positional refill, discrete five-tap rounds, and a character-owned Hunger display made the game feel substantially clearer and more enjoyable. The new loop preserves tactile repeated tapping and the strongest positional identities while removing machine topology, movement bookkeeping, and incremental progression from the immediate decision. Five renewable taps provide a readable action economy; Grandma's telegraphed response provides a round boundary; and cups turn opening an egg into a visible routing decision. The accepted baseline is intentionally smaller so human play can determine whether players reject immediate Yolk to construct stronger later combinations before more content is added.
