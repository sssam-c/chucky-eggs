...
The complete deterministic and player-visible path is implemented and covered by the automated suite. The starting economy uses five eggs, ten thwacks, an 8-point opening target, and a 9-point later target. Bin recycling now waits for the conveyor to clear instead of feeding missed eggs back into a live wave one at a time. The next worthwhile work is a short three-day playtest. No recorded playtest yet establishes whether this budget leaves meaningful recovery room, whether the bin creates intentional repositioning rather than passive delay, or whether tougher merged eggs feel like valuable concentrated projects rather than chores.
