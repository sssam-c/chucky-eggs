...
- `egg-symbology-roster-concept-v4.png`: the source hierarchy for the current egg-face syntax, retained with superseded species-coloured shells, bird-icon placement, and pointed Yolk drops. The large central toughness number is the sole temporal divider: optional Tap effects appear directly above it, a large translucent species silhouette sits behind it, and Yolk plus any additional On Break instructions form a frameless payload row at the base. Each Yolk value is centred inside a regular rounded blob with two-digit capacity. Woodpecker is the first implemented On Break instruction, using a spoon-plus-right-arrow mark beside its Yolk. The implemented placeholder shells are uniformly white, and circuit colour is reserved for genuinely colour-gated marks.
