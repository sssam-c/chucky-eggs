...
- `egg-symbology-roster-concept-v4.png`: the source hierarchy for the current egg-face syntax, retained with superseded species-coloured shells and bird-icon placement. The large central toughness number is the sole temporal divider: optional Tap effects appear directly above it, a large translucent species silhouette sits behind it, and Yolk plus any additional On Crack instructions form a frameless payload row at the base. Each Yolk value is contained inside its drop. The implemented placeholder shells are uniformly white, and circuit colour is reserved for genuinely colour-gated marks.
