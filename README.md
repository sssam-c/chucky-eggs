...
> **Branch prototype:** `codex/hopper-tap-combos` launches a branch-only five-cup tabletop experiment. Click an egg's coloured spoon to tap it; opening an egg visibly drops the next hopper egg into that same cup after the complete tap cascade. Grandma waits beside the table and owns the visible Hunger and next-increase display. Five paid taps form a Tap phase. If Yolk has not reduced her Hunger to zero, her telegraphed Hunger increase resolves and the next five taps begin. The canonical conveyor game remains available at `res://src/ui/main.tscn` and its settled rules are unchanged pending playtest evidence.
