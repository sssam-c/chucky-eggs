...
const CONVEYOR_ENTRY_LOCAL := Vector2(216.0, 432.0)
const HOPPER_OUTLET_LOCAL := Vector2(216.0, 432.0)
const CONVEYOR_PANEL_RECT := Rect2(198.0, 410.0, 700.0, 96.0)
const BIN_VISUAL_RECT := Rect2(806.0, 526.0, 140.0, 162.0)
...
func _draw_straight_machine() -> void:
	...
	draw_line(Vector2(203, 422), Vector2(893, 422), Color("ad6532"), 3.0)
	for arrow_x in [346.0, 484.0, 622.0, 760.0]:
		_draw_route_arrow(Vector2(arrow_x, 435.0), Vector2.RIGHT)
	_draw_rollers(224, 892, 481)
	...
	draw_rect(Rect2(816, 542, 120, 132), Color("160d08"), true)
	draw_line(Vector2(812, 540), Vector2(940, 540), Color("e0a044"), 7.0, true)
	draw_line(Vector2(828, 552), Vector2(838, 674), Color("2a160c"), 5.0, true)
	draw_line(Vector2(924, 552), Vector2(914, 674), Color("2a160c"), 5.0, true)
	...
	_draw_iron_panel(Rect2(206, 510, 594, 186), Color("754126"))
	draw_line(Vector2(208, 512), Vector2(798, 512), Color("9f5529"), 4.0)
	for rail_x in [208.0, 405.0, 603.0, 798.0]:
		draw_rect(Rect2(rail_x, 518, 10, 168), Color("2b2b2c"), true)
		draw_line(Vector2(rail_x + 3, 521), Vector2(rail_x + 3, 682), Color("754126"), 2.0)
	draw_line(Vector2(220, 518), Vector2(386, 518), Color("c43b36"), 4.0)
	draw_line(Vector2(418, 518), Vector2(584, 518), Color("287cbd"), 4.0)
	draw_line(Vector2(616, 518), Vector2(782, 518), Color("cf4f8b"), 4.0)
	for bolt in [Vector2(226, 530), Vector2(395, 530), Vector2(593, 530), Vector2(788, 530), Vector2(226, 677), Vector2(395, 677), Vector2(593, 677), Vector2(788, 677)]:
...
func _draw_curved_bin_exit() -> void:
	...
	for stroke in [
		...
		draw_line(
			Vector2(943, 495), Vector2(943, 532),
			stroke.color, stroke.width, true
		)
	draw_line(Vector2(897, 450), Vector2(929, 465), Color("e0a044"), 3.0, true)
	_draw_route_arrow(Vector2(943, 511), Vector2.DOWN)
...
func _draw_straight_belt_sections() -> void:
	var boundaries := [202.0, 346.0, 484.0, 622.0, 760.0, 898.0]
...
func _draw_supply_hopper() -> void:
	var lift_bounds := Rect2(4, 306, 226, 390)
	_draw_contact_shadow(lift_bounds, 8.0)

	# The hopper is a floor-mounted lift magazine. Its tray shares the bin's
	# baseline, and the visible queue rises toward the conveyor-level exit.
	var magazine := Rect2(18, 366, 184, 286)
	draw_rect(magazine, Color("111719"), true)
	draw_rect(magazine.grow(-5.0), Color(0.07, 0.31, 0.32, 0.64), true)
	draw_line(Vector2(32, 377), Vector2(32, 640), Color(0.35, 0.76, 0.74, 0.30), 4.0, true)
	draw_rect(magazine, Color("2d7d7e"), false, 5.0)

	# A short top throat hands the raised egg to bay one. It meets the conveyor
	# directly and never implies another queue position.
	var outlet := PackedVector2Array([
		Vector2(18, 366), Vector2(202, 366), Vector2(210, 410),
		Vector2(230, 410), Vector2(230, 454), Vector2(202, 454),
		Vector2(190, 414), Vector2(18, 414),
	])
	...
	draw_polyline(
		PackedVector2Array([
			Vector2(18, 366), Vector2(202, 366), Vector2(210, 410),
			Vector2(230, 410), Vector2(230, 454), Vector2(202, 454),
			Vector2(190, 414), Vector2(18, 414), Vector2(18, 366),
		]),
		Color("8b512e"),
		5.0,
		true
	)
	draw_line(Vector2(207, 423), Vector2(228, 423), Color("e0a044"), 3.0, true)

	# The heavy floor deck is the hopper's loading tray and aligns visually with
	# the discard bin on the opposite side of the controls.
	var deck := PackedVector2Array([
		Vector2(18, 628), Vector2(202, 628), Vector2(218, 648),
		Vector2(218, 692), Vector2(4, 692), Vector2(4, 648),
	])
	draw_colored_polygon(deck, Color("252629"))
	draw_polyline(
		PackedVector2Array([
			Vector2(18, 628), Vector2(202, 628), Vector2(218, 648),
			Vector2(218, 692), Vector2(4, 692), Vector2(4, 648),
	...
	draw_line(Vector2(14, 682), Vector2(208, 682), Color("e0a044"), 3.0, true)

	# Upward chevrons explain the reversed physical orientation without adding
	# queue labels or changing the underlying preview facts.
	for arrow_y in [592.0, 505.0, 418.0]:
		_draw_route_arrow(Vector2(211, arrow_y), Vector2.UP)

	for bolt in [Vector2(26, 374), Vector2(194, 374), Vector2(26, 644), Vector2(194, 644)]:
...
func _draw_spoon_mount_shadows() -> void:
	for spoon_index in range(5):
		var center := Vector2(277.0 + spoon_index * 138.0, 172.0)
